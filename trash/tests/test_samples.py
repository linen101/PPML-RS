...
def test_samples(n_values, n, d, alpha, sigma, test_perc, delta, m_estimators, method_labels, max_iterations, error_tolerance, M ,eta, iters_sever = 5, w_star=None):
    # Initialize iteration parameter
    i = 0    
    
    # Initialize weight  arrays
    w_star = np.empty(len(n_values), dtype=object)
    w_robust_M = np.empty((len(n_values), len(m_estimators)), dtype=object)
    w_robust_STIR = np.empty(len(n_values), dtype=object)
    w_robust_STIR_ideal = np.empty(len(n_values), dtype=object)
    w_TORRENT = np.empty(len(n_values), dtype=object)
    w_TORRENT_ideal = np.empty(len(n_values), dtype=object)
    w_svam = np.empty(len(n_values), dtype=object)
    w_svam_ideal = np.empty(len(n_values), dtype=object)
    w_sever = np.empty(len(n_values), dtype=object)

    
    # Initialize  error arrays
    error_M = np.empty((len(n_values), len(m_estimators)), dtype=object)
    error_STIR = np.empty(len(n_values), dtype=object)
    error_STIR_ideal = np.empty(len(n_values), dtype=object)
    error_TORRENT = np.empty(len(n_values), dtype=object)
    error_TORRENT_ideal = np.empty(len(n_values), dtype=object)
    error_svam = np.empty(len(n_values), dtype=object)
    error_svam_ideal = np.empty(len(n_values), dtype=object)
    error_sever = np.empty(len(n_values), dtype=object)

    
    # Initialize iterations arrays for each method
    iters_M = np.empty((len(n_values), len(m_estimators)), dtype=object)
    iters_STIR = np.empty(len(n_values), dtype=object)
    iters_TORRENT = np.empty(len(n_values), dtype=object)
    iters_svam = np.empty(len(n_values), dtype=object)
    
    #Initialize ideal iterations arrays for each method
    iters_M_ideal = np.empty((len(n_values), len(m_estimators)), dtype=object)
    iters_STIR_ideal = np.empty(len(n_values), dtype=object)
    iters_TORRENT_ideal = np.empty(len(n_values), dtype=object)
    iters_svam_ideal = np.empty(len(n_values), dtype=object)
    
    # Create synthetic dataset and model and get estimated weights 
    # for each n in n_values array, representing the number of samples
    # while dimensions, corruption rate and gaussian noise remain constant
    for num in n_values:
        X_train, Y_train, X_test, Y_test, w_star[i] = generate_synthetic_dataset(num, d, sigma, test_perc)   
        ...
        beta = 0.2          # ratio of outliers to be removed
    ...
    plot_regression_errors_n(errors, n_values, d, alpha, sigma, method_labels, error_tolerance, M ,eta)
    ...
    plot_iterations_n(iters, n_values, d, alpha, sigma, method_labels_new, error_tolerance, M ,eta)
    '''
